 ...
 * are smaller than the root specified
 * @node: node in the tree to verify the condition
 * @max: value to compare
 * Return: 1 if all nodes small or equal or 0 if not
 */
...
/**
 * check_sub_tree_Right - Must check if all the nodes are 
 * bigger than the root specified
 * @node: to verify condition
 * @min: value to compare
 * Return: 1 if all is big or equal or 0 if not
 */
